    ...
    unsigned long scope_id, boost::system::error_code& ec)
{
  ...
  {
    nvx_sprintf_s(dest, length, "%u.%u.%u.%u",
  ...
  else if (af == BOOST_ASIO_OS_DEF(AF_INET6))
  {
    ...
    while (n < length && b < 16)
    {
      if (bytes[b] == 0 && bytes[b + 1] == 0 && z == 0)
      {
        ...
        n += nvx_sprintf_s(dest + n, length - n, ":%s", b < 16 ? "" : ":"), ++z;
      }
      else
      {
        n += nvx_sprintf_s(dest + n, length - n, "%s%x", b ? ":" : "",
            (static_cast<u_long_type>(bytes[b]) << 8) | bytes[b + 1]);
        b += 2;
      }
    }
    if (scope_id)
      n += nvx_sprintf_s(dest + n, length - n, "%%%lu", scope_id);
    return dest;
  }
  else
  {
    ...
    unsigned long* scope_id, boost::system::error_code& ec)
{
  ...
  if (af == BOOST_ASIO_OS_DEF(AF_INET))
  {
    ...
    if (nvx_snvx_scanf_s(src, "%u.%u.%u.%u", &b0, &b1, &b2, &b3) != 4)
    {
      ec = boost::asio::error::invalid_argument;
      return -1;
    }
  ...
  try
  {
    ...
    for (unsigned i = 0; i < hostnames->Size; ++i)
    {
      ...
      if (hostname->Type == HostNameType::DomainName)
      {
        ...
        {
          nvx_strcpy_s(name, namelen, raw_name.c_str());
          return 0;
        }
      }
    }
...
inline void gai_strcpy(char* target, const char* source, std::size_t max_size)
{
  ...
  nvx_strcpy_s(target, max_size, source);
...
#if defined(BOOST_ASIO_HAS_SECURE_RTL)
      nvx_sprintf_s(serv, servlen, "%u", ntohs(port));
#else // defined(BOOST_ASIO_HAS_SECURE_RTL)
