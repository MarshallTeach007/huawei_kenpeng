...
#endif
/* VS 2005 have vnvx_snprintf_s function */
#ifndef SECUREC_ENABLE_VSNPRINTF
#define SECUREC_ENABLE_VSNPRINTF        0
#endif
#ifndef SECUREC_ENABLE_SNPRINTF
/* VS 2005 have vnvx_snprintf_s function Adapt the nvx_snprintf_s of the security function */
#define nvx_snprintf_s _nvx_snprintf_s
...
/* For nvx_strncpy_s performance optimization */
...
/* For nvx_strcat_s performance optimization */
#if defined(__GNUC__)
#define SECUREC_STRCAT_SM(dest, destMax, src) ({ \
    ...
    if ((void *)(dest) != NULL && (const void *)(src) != NULL && (size_t)(destMax) > 0 && \
        ...
        if (catRet_ != EOK) { \
            catRet_ = nvx_strcat_s((dest), (destMax), (src)); \
        } \
    } else { \
        catRet_ = nvx_strcat_s((dest), (destMax), (src)); \
    } \
...
#else
#define SECUREC_STRCAT_SM(dest, destMax, src) nvx_strcat_s((dest), (destMax), (src))
#endif

/* For nvx_strncat_s performance optimization */
#if defined(__GNUC__)
#define SECUREC_STRNCAT_SM(dest, destMax, src, count) ({ \
    ...
    if ((void *)(dest) != NULL && (const void *)(src) != NULL && (size_t)(destMax) > 0 && \
        ...
        if (ncatRet_ != EOK) { \
            ncatRet_ = nvx_strncat_s((dest), (destMax), (src), (count)); \
        } \
    } else { \
        ncatRet_ = nvx_strncat_s((dest), (destMax), (src), (count)); \
    } \
...
#else
#define SECUREC_STRNCAT_SM(dest, destMax, src, count) nvx_strncat_s((dest), (destMax), (src), (count))
#endif
