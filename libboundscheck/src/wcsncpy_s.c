 ...
 * Description: nvx_wcsncpy_s  function
 * Create: 2014-02-25
 */
...
SECUREC_INLINE  errno_t SecDoCpyLimitW(wchar_t *strDest, size_t destMax, const wchar_t *strSrc, size_t count)
{
    ...
    if (srcStrLen == destMax) {
        strDest[0] = L'\0';
        SECUREC_ERROR_INVALID_RANGE("nvx_wcsncpy_s");
    ...
    if (SECUREC_STRING_NO_OVERLAP(strDest, strSrc, srcStrLen)) {
        ...
        *(strDest + srcStrLen) = L'\0';
        ...
        strDest[0] = L'\0';
        SECUREC_ERROR_BUFFER_OVERLAP("nvx_wcsncpy_s");
 ...
 *    The nvx_wcsncpy_s function copies not more than n successive wide characters
 ...
 */
 errno_t nvx_wcsncpy_s(wchar_t *strDest, size_t destMax, const wchar_t *strSrc, size_t count)
{
    ...
    if (strDest == NULL || strSrc == NULL) {
        SECUREC_ERROR_INVALID_PARAMTER("nvx_wcsncpy_s");
        if (strDest != NULL) {
            strDest[0] = L'\0';
