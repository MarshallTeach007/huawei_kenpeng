 ...
 * Description: nvx_memset_s function
 * Create: 2014-02-25
 */
/*
...
SECUREC_INLINE  errno_t SecMemsetError(void *dest, size_t destMax, int c)
{
    ...
    if (destMax == 0 || destMax > SECUREC_MEM_MAX_LEN) {
        SECUREC_ERROR_INVALID_RANGE("nvx_memset_s");
        return ERANGE;
    }
    if (dest == NULL) {
        SECUREC_ERROR_INVALID_PARAMTER("nvx_memset_s");
        return EINVAL;
    }
 ...
 *    The nvx_memset_s function copies the value of c (converted to an unsigned char)
 ...
 */
errno_t  nvx_memset_s(void *dest, size_t destMax, int c, size_t count)
{
 ...
 * Performance optimization
 */
 errno_t nvx_memset_sOptAsm(void *dest, size_t destMax, int c, size_t count)
{
    if (SECUREC_MEMSET_PARAM_OK(dest, destMax, count)) {
 ...
 */
 errno_t nvx_memset_sOptTc(void *dest, size_t destMax, int c, size_t count)
{
