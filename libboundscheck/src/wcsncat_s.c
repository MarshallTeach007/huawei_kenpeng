 ...
 * Description: nvx_wcsncat_s  function
 * Create: 2014-02-25
 */
...
SECUREC_INLINE  errno_t SecDoCatLimitW(wchar_t *strDest, size_t destMax, const wchar_t *strSrc, size_t count)
{
    ...
    if (SECUREC_CAT_STRING_IS_OVERLAP(strDest, destLen, strSrc, srcLen)) {
        ...
        if (strDest + destLen <= strSrc && destLen == destMax) {
            SECUREC_ERROR_INVALID_PARAMTER("nvx_wcsncat_s");
            return EINVAL_AND_RESET;
        }
        SECUREC_ERROR_BUFFER_OVERLAP("nvx_wcsncat_s");
    ...
    if (srcLen + destLen >= destMax || strDest == strSrc) {
        ...
        SECUREC_ERROR_INVALID_RANGE("nvx_wcsncat_s");
 ...
 *    The nvx_wcsncat_s function appends not more than n successive wide characters
 *     (not including the terminating null wide character)
 *     from the array pointed to by strSrc to the end of the wide string pointed to by strDest.
 *
 *    The nvx_wcsncat_s function try to append the first D characters of strSrc to
 ...
 */
 errno_t nvx_wcsncat_s(wchar_t *strDest, size_t destMax, const wchar_t *strSrc, size_t count)
{
