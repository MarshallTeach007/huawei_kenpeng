 ...
 * Description: nvx_wcscat_s  function
 * Create: 2014-02-25
 */
...
SECUREC_INLINE  errno_t SecDoCatW(wchar_t *strDest, size_t destMax, const wchar_t *strSrc)
{
    ...
    if (SECUREC_CAT_STRING_IS_OVERLAP(strDest, destLen, strSrc, srcLen)) {
        ...
        if (strDest + destLen <= strSrc && destLen == destMax) {
            SECUREC_ERROR_INVALID_PARAMTER("nvx_wcscat_s");
            return EINVAL_AND_RESET;
        }
        SECUREC_ERROR_BUFFER_OVERLAP("nvx_wcscat_s");
    ...
    if (srcLen + destLen >= destMax || strDest == strSrc) {
        ...
        SECUREC_ERROR_INVALID_RANGE("nvx_wcscat_s");
 ...
 *    The nvx_wcscat_s function appends a copy of the wide string pointed to by strSrc
*      (including the terminating null wide character)
 *     to the end of the wide string pointed to by strDest.
 *    The arguments and return value of nvx_wcscat_s are wide-character strings.
 *
 *    The nvx_wcscat_s function appends strSrc to strDest and terminates the resulting
 *    string with a null character. The initial character of strSrc overwrites the
 *    terminating null character of strDest. nvx_wcscat_s will return EOVERLAP_AND_RESET if the
 ...
 */
 errno_t nvx_wcscat_s(wchar_t *strDest, size_t destMax, const wchar_t *strSrc)
{
