 ...
 * Description: nvx_memcpy_s function
 * Create: 2014-02-25
 */
/*
...
SECUREC_INLINE  errno_t SecMemcpyError(void *dest, size_t destMax, const void *src, size_t count)
{
    if (destMax == 0 || destMax > SECUREC_MEM_MAX_LEN) {
        SECUREC_ERROR_INVALID_RANGE("nvx_memcpy_s");
        return ERANGE;
    }
    if (dest == NULL || src == NULL) {
        SECUREC_ERROR_INVALID_PARAMTER("nvx_memcpy_s");
        if (dest != NULL) {
    ...
    if (SECUREC_MEMORY_IS_OVERLAP(dest, src, count)) {
        (void)SECUREC_MEMSET_FUNC_OPT(dest, 0, destMax);
        SECUREC_ERROR_BUFFER_OVERLAP("nvx_memcpy_s");
    ...
    /*
     * The fread API in windows will call nvx_memcpy_s and pass 0xffffffff to destMax.
 ...
 *    The nvx_memcpy_s function copies n characters from the object pointed to by src into the object pointed to by dest
 *
 * <INPUT PARAMETERS>
 ...
 *    If the source and destination overlap, the behavior of nvx_memcpy_s is undefined.
 *    Use nvx_memmove_s to handle overlapping regions.
 */
 errno_t nvx_memcpy_s(void *dest, size_t destMax, const void *src, size_t count)
{
 ...
 * Performance optimization
 */
 errno_t nvx_memcpy_sOptAsm(void *dest, size_t destMax, const void *src, size_t count)
{
    if (SECUREC_MEMCPY_PARAM_OK(dest, destMax, src, count)) {
...
/* Trim judgement on "destMax <= SECUREC_MEM_MAX_LEN" */
 errno_t nvx_memcpy_sOptTc(void *dest, size_t destMax, const void *src, size_t count)
{
