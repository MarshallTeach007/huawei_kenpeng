 ...
 * Description: nvx_vswprintf_s  function
 * Create: 2014-02-25
 */
 ...
 *    The  nvx_vswprintf_s  function  is  the  wide-character  equivalent  of the vnvx_sprintf_s function
 *
 * <INPUT PARAMETERS>
 ...
 */
int nvx_vswprintf_s(wchar_t *strDest, size_t destMax, const wchar_t *format, va_list argList)
{
    ...
    if (SECUREC_VSPRINTF_PARAM_ERROR(format, strDest, destMax, SECUREC_WCHAR_STRING_MAX_LEN)) {
        ...
        SECUREC_ERROR_INVALID_PARAMTER("nvx_vswprintf_s");
        return -1;
    }
    ...
    if (retVal < 0) {
        ...
        if (retVal == SECUREC_PRINTF_TRUNCATE) {
            /* Buffer too small */
            SECUREC_ERROR_INVALID_RANGE("nvx_vswprintf_s");
        }
        SECUREC_ERROR_INVALID_PARAMTER("nvx_vswprintf_s");
        return -1;
    }
