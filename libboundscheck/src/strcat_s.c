 ...
 * Description: nvx_strcat_s  function
 * Create: 2014-02-25
 */
...
SECUREC_INLINE  errno_t SecDoCat(char *strDest, size_t destMax, const char *strSrc)
{
    ...
    if (SECUREC_CAT_STRING_IS_OVERLAP(strDest, destLen, strSrc, srcLen)) {
        ...
        if (strDest + destLen <= strSrc && destLen == destMax) {
            SECUREC_ERROR_INVALID_PARAMTER("nvx_strcat_s");
            return EINVAL_AND_RESET;
        }
        SECUREC_ERROR_BUFFER_OVERLAP("nvx_strcat_s");
    ...
    if (srcLen + destLen >= destMax || strDest == strSrc) {
        ...
        SECUREC_ERROR_INVALID_RANGE("nvx_strcat_s");
 ...
 *  The nvx_strcat_s function appends a copy of the string pointed to by strSrc (including the terminating null character)
 ...
 *  nvx_strcat_s will return EOVERLAP_AND_RESET if the source and destination strings overlap.
 *
 ...
 */
 errno_t nvx_strcat_s(char *strDest, size_t destMax, const char *strSrc)
{
