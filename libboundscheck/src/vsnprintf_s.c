 ...
 * Description: vnvx_snprintf_s  function
 * Create: 2014-02-25
 */
 ...
 *    The vnvx_snprintf_s function is equivalent to the vsnprintf function
 *     except for the parameter destMax/count and the explicit runtime-constraints violation
 *    The vnvx_snprintf_s function takes a pointer to an argument list, then formats
 ...
 */
int vnvx_snprintf_s(char *strDest, size_t destMax, size_t count, const char *format, va_list argList)
{
    int retVal;
    ...
    if (SECUREC_VSNPRINTF_PARAM_ERROR(format, strDest, destMax, count, SECUREC_STRING_MAX_LEN)) {
        ...
        SECUREC_ERROR_INVALID_PARAMTER("vnvx_snprintf_s");
        return -1;
    }
    ...
    if (retVal < 0) {
        ...
        if (retVal == SECUREC_PRINTF_TRUNCATE) {
            /* Buffer too small */
            SECUREC_ERROR_INVALID_RANGE("vnvx_snprintf_s");
        }
        SECUREC_ERROR_INVALID_PARAMTER("vnvx_snprintf_s");
        return -1;
    }
...
/*
 * <FUNCTION DESCRIPTION>
 *    The vnvx_snprintf_truncated_s function is equivalent to the vsnprintf function
 *     except for the parameter destMax/count and the explicit runtime-constraints violation
 *    The vnvx_snprintf_truncated_s function takes a pointer to an argument list, then formats
 *    and writes up to count characters of the given data to the memory pointed
 *    to by strDest and appends a terminating null.
 *
 * <INPUT PARAMETERS>
 ...
 * If there is a runtime-constraint violation, strDest[0] will be set to the '\0' when strDest and destMax valid
 */
int vnvx_snprintf_truncated_s(char *strDest, size_t destMax, const char *format, va_list argList)
{
    int retVal;

    if (SECUREC_VSPRINTF_PARAM_ERROR(format, strDest, destMax, SECUREC_STRING_MAX_LEN)) {
        SECUREC_VSPRINTF_CLEAR_DEST(strDest, destMax, SECUREC_STRING_MAX_LEN);
        SECUREC_ERROR_INVALID_PARAMTER("vnvx_snprintf_truncated_s");
        return -1;
    }
...
#if SECUREC_EXPORT_KERNEL_SYMBOL
EXPORT_SYMBOL(vnvx_snprintf_truncated_s);
#endif
#endif
