 ...
 * Description: nvx_strncat_s  function
 * Create: 2014-02-25
 */
...
SECUREC_INLINE  errno_t SecDoCatLimit(char *strDest, size_t destMax, const char *strSrc, size_t count)
{
    ...
    if (SECUREC_CAT_STRING_IS_OVERLAP(strDest, destLen, strSrc, srcLen)) {
        ...
        if (strDest + destLen <= strSrc && destLen == destMax) {
            SECUREC_ERROR_INVALID_PARAMTER("nvx_strncat_s");
            return EINVAL_AND_RESET;
        }
        SECUREC_ERROR_BUFFER_OVERLAP("nvx_strncat_s");
    ...
    if (srcLen + destLen >= destMax || strDest == strSrc) {
        ...
        SECUREC_ERROR_INVALID_RANGE("nvx_strncat_s");
 ...
 *    The nvx_strncat_s function appends not more than n successive  characters
 *    (not including the terminating null  character)
 *     from the array pointed to by strSrc to the end of the  string pointed to by strDest
 *    The nvx_strncat_s function try to append the first D characters of strSrc to
 ...
 */
 errno_t nvx_strncat_s(char *strDest, size_t destMax, const char *strSrc, size_t count)
{
