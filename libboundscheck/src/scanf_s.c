 ...
 * Description: nvx_scanf_s function
 * Create: 2014-02-25
 */
 ...
 *    The nvx_scanf_s function is equivalent to fnvx_scanf_s with the argument stdin interposed before the arguments to nvx_scanf_s
 *     The nvx_scanf_s function reads data from the standard input stream stdin and
 ...
 */
int nvx_scanf_s(const char *format, ...)
{
    int ret;                    /* If initialization causes  e838 */
    va_list argList;

    va_start(argList, format);
    ret = vnvx_scanf_s(format, argList);
    va_end(argList);
