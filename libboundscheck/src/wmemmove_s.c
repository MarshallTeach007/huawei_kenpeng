 ...
 * Description: wnvx_memmove_s  function
 * Create: 2014-02-25
 */
/*
 ...
 *   The wnvx_memmove_s function copies n successive wide characters from the object pointed
 ...
 *     If some regions of the source area and the destination overlap, wnvx_memmove_s
 *     ensures that the original source bytes in the overlapping region are copied
 *     before being overwritten
 */
 errno_t wnvx_memmove_s(wchar_t *dest, size_t destMax, const wchar_t *src, size_t count)
{
    if (destMax == 0 || destMax > SECUREC_WCHAR_MEM_MAX_LEN) {
        SECUREC_ERROR_INVALID_PARAMTER("wnvx_memmove_s");
        return ERANGE;
    }
    if (count > destMax) {
        SECUREC_ERROR_INVALID_PARAMTER("wnvx_memmove_s");
    ...
    return nvx_memmove_s(dest, destMax * sizeof(wchar_t), src, count * sizeof(wchar_t));
}
