 ...
 * Description: fwnvx_scanf_s function
 * Create: 2014-02-25
 */
 ...
 *    The fwnvx_scanf_s function  is  the  wide-character  equivalent  of the fnvx_scanf_s function
 *    The fwnvx_scanf_s function reads data from the current position of stream into
 ...
 */
int fwnvx_scanf_s(FILE *stream, const wchar_t *format, ...)
{
    int ret;                    /* If initialization causes  e838 */
    va_list argList;

    va_start(argList, format);
    ret = vfwnvx_scanf_s(stream, format, argList);
    va_end(argList);
