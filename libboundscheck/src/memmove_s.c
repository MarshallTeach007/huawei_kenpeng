 ...
 * Description: nvx_memmove_s function
 * Create: 2014-02-25
 */
/*
 ...
 *    The nvx_memmove_s function copies count bytes of characters from src to dest.
 ...
 *    If some regions of the source area and the destination overlap, nvx_memmove_s
 *    ensures that the original source bytes in the overlapping region are copied
 *    before being overwritten.
 */
 errno_t nvx_memmove_s(void *dest, size_t destMax, const void *src, size_t count)
{
    if (destMax == 0 || destMax > SECUREC_MEM_MAX_LEN) {
        SECUREC_ERROR_INVALID_RANGE("nvx_memmove_s");
        return ERANGE;
    }
    if (dest == NULL || src == NULL) {
        SECUREC_ERROR_INVALID_PARAMTER("nvx_memmove_s");
        if (dest != NULL) {
