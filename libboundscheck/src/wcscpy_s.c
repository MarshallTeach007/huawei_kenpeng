 ...
 * Description: nvx_wcscpy_s  function
 * Create: 2014-02-25
 */
...
SECUREC_INLINE  errno_t SecDoCpyW(wchar_t *strDest, size_t destMax, const wchar_t *strSrc)
{
    ...
    if (srcStrLen == destMax) {
        strDest[0] = L'\0';
        SECUREC_ERROR_INVALID_RANGE("nvx_wcscpy_s");
    ...
    if (SECUREC_STRING_NO_OVERLAP(strDest, strSrc, srcStrLen)) {
        ...
        strDest[0] = L'\0';
        SECUREC_ERROR_BUFFER_OVERLAP("nvx_wcscpy_s");
 ...
 *   The nvx_wcscpy_s function copies the wide string pointed to by strSrc
 ...
 */
 errno_t nvx_wcscpy_s(wchar_t *strDest, size_t destMax, const wchar_t *strSrc)
{
    ...
    if (strDest == NULL || strSrc == NULL) {
        SECUREC_ERROR_INVALID_PARAMTER("nvx_wcscpy_s");
        if (strDest != NULL) {
            strDest[0] = L'\0';
