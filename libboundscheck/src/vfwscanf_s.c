 ...
 * Description: vfwnvx_scanf_s  function
 * Create: 2014-02-25
 */
 ...
 *    The  vfwnvx_scanf_s  function  is  the  wide-character  equivalent  of the vfnvx_scanf_s function
 *    The vfwnvx_scanf_s function reads data from the current position of stream into
 ...
 */
int vfwnvx_scanf_s(FILE *stream, const wchar_t *format, va_list argList)
{
    int retVal; /* If initialization causes  e838 */
    SecFileStream fStr;

    if (stream == NULL || format == NULL) {
        SECUREC_ERROR_INVALID_PARAMTER("vfwnvx_scanf_s");
        return SECUREC_SCANF_EINVAL;
    }
    if (stream == SECUREC_STREAM_STDIN) {
        return vwnvx_scanf_s(format, argList);
    }
