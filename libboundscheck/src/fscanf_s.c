 ...
 * Description: fnvx_scanf_s function
 * Create: 2014-02-25
 */
 ...
 *    The fnvx_scanf_s function is equivalent to fscanf except that the c, s,
 ...
 */
__attribute__ ((visibility ("hidden"))) int fnvx_scanf_s(FILE *stream, const char *format, ...)
{
    int ret;                    /* If initialization causes  e838 */
    va_list argList;

    va_start(argList, format);
    ret = vfnvx_scanf_s(stream, format, argList);
    va_end(argList);
