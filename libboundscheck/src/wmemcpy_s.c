 ...
 * Description: wnvx_memcpy_s  function
 * Create: 2014-02-25
 */
/*
 ...
 *   The wnvx_memcpy_s function copies n successive wide characters
 ...
 *    If the source and destination overlap, the behavior of wnvx_memcpy_s is undefined.
 *    Use wnvx_memmove_s to handle overlapping regions.
 */
 errno_t wnvx_memcpy_s(wchar_t *dest, size_t destMax, const wchar_t *src, size_t count)
{
    if (destMax == 0 || destMax > SECUREC_WCHAR_MEM_MAX_LEN) {
        SECUREC_ERROR_INVALID_PARAMTER("wnvx_memcpy_s");
        return ERANGE;
    }
    if (count > destMax) {
        SECUREC_ERROR_INVALID_PARAMTER("wnvx_memcpy_s");
    ...
    return nvx_memcpy_s(dest, destMax * sizeof(wchar_t), src, count * sizeof(wchar_t));
}
