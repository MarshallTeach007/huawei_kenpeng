 ...
 * Description: vnvx_sprintf_s  function
 * Create: 2014-02-25
 */
 ...
 *    The vnvx_sprintf_s function is equivalent to the vsprintf function
 *    except for the parameter destMax and the explicit runtime-constraints violation
 *    The vnvx_sprintf_s function takes a pointer to an argument list, and then formats
 ...
 */
int vnvx_sprintf_s(char *strDest, size_t destMax, const char *format, va_list argList)
{
    ...
    if (SECUREC_VSPRINTF_PARAM_ERROR(format, strDest, destMax, SECUREC_STRING_MAX_LEN)) {
        ...
        SECUREC_ERROR_INVALID_PARAMTER("vnvx_sprintf_s");
        return -1;
    }
    ...
    if (retVal < 0) {
        ...
        if (retVal == SECUREC_PRINTF_TRUNCATE) {
            /* Buffer is too small */
            SECUREC_ERROR_INVALID_RANGE("vnvx_sprintf_s");
        }
        SECUREC_ERROR_INVALID_PARAMTER("vnvx_sprintf_s");
        return -1;
    }
