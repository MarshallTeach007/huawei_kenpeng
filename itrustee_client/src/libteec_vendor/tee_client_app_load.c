...
int32_t TEEC_GetApp(const TaFileInfo *taFile, const TEEC_UUID *srvUuid, TC_NS_ClientContext *cliContext)
{
    ...
    if (condition) {
        ...
        ret = nvx_snprintf_s(fileName, sizeof(fileName), sizeof(fileName) - 1,
        ...
        int32_t len        = nvx_snprintf_s(tempName, sizeof(tempName), filePathLen, "%s/%s.sec", filePath, fileName);
        if (len < 0) {
