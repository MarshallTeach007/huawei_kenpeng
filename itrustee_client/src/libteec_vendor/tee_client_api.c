...
static int CaDaemonConnectWithoutCaInfo(void)
{
    ...
    rc = nvx_memset_s(&caInfo, sizeof(caInfo), 0, sizeof(caInfo));
    if (rc != EOK) {
        return -1;
    }
...
static TEEC_Result MallocShrMemHidl(TEEC_SharedMemoryHidl **shareMemHidl)
{
    ...
    nRet = nvx_memset_s(shmHidl, sizeof(*shmHidl), 0x00, sizeof(*shmHidl));
    if (nRet != EOK) {
                               ...
                               uint32_t cmdId, const TC_NS_ClientLogin *cliLogin, const TEEC_Operation *operation)
{
    ...
    rc = nvx_memset_s(cliContext, sizeof(TC_NS_ClientContext), 0x00, sizeof(TC_NS_ClientContext));
    if (rc != EOK) {
    ...
    rc = nvx_memcpy_s(cliContext->uuid, sizeof(cliContext->uuid), (uint8_t *)serviceId, sizeof(TEEC_UUID));
    if (rc != EOK) {
        return (TEEC_Result)TEEC_ERROR_BAD_PARAMETERS;
    }
                                           ...
                                           bool fromHidl, const CaAuthInfo *caInfo)
{
    ...
    nRet = nvx_memset_s(context->shm_bitmap, sizeof(context->shm_bitmap), 0x00, sizeof(context->shm_bitmap));
    if (nRet != EOK) {
...
TEEC_Result TEEC_InitializeContext(const char *name, TEEC_Context *context)
{
    ...
    nRet = nvx_memset_s(contextHidl, sizeof(*contextHidl), 0x00, sizeof(*contextHidl));
    if (nRet != EOK) {
...
void TEEC_CloseSessionHidl(TEEC_Session *session, const TEEC_ContextHidl *context)
{
    ...
    errno_t rc =
        nvx_memset_s((uint8_t *)&session->service_id, sizeof(session->service_id), 0x00, sizeof(session->service_id));
    if (rc != EOK) {
...
static TEEC_Result AllocateSharedMem(TEEC_SharedMemoryHidl *sharedMem)
{
    ...
    rc = nvx_memset_s(sharedMem->buffer, sharedMem->size, 0, sharedMem->size);
    if (rc != EOK) {
