...
static int ConnectTeecdSocket(int *socketFd)
{
    ...
    errno_t rc = nvx_memset_s(&remote, sizeof(remote), 0, sizeof(remote));
    if (rc != EOK) {
        return -1;
    }
    ...
    rc = nvx_strncpy_s(remote.sun_path, sizeof(remote.sun_path), TC_NS_SOCKET_NAME, sizeof(TC_NS_SOCKET_NAME));
    if (rc != EOK) {
...
static int RecvFileDescriptor(int cmd, int socketFd)
{
    ...
    rc = nvx_memset_s(&hmsg, sizeof(struct msghdr), 0, sizeof(struct msghdr));
    if (rc != EOK) {
        return -1;
    }
    rc = nvx_memset_s(ctrlBuf, sizeof(ctrlBuf), 0, CMSG_SPACE(sizeof(int)));
    if (rc != EOK) {
        return -1;
    }
    /* For the dummy data */
...
static int FillMsgBuffer(const CaAuthInfo *caInfo, CaRevMsg **revMsg, int cmd, const TEEC_XmlParameter *halXmlPtr)
{
    ...
    errno_t ret = nvx_memset_s((void *)revBuffer, sizeof(*revBuffer), 0, sizeof(*revBuffer));
    if (ret != EOK) {
        tloge("memset failed\n");
        free(revBuffer);
        return -1;
    }
    int temp = nvx_memcpy_s(&(revBuffer->caAuthInfo), sizeof(CaAuthInfo), caInfo, sizeof(*caInfo));
    if (temp != EOK) {
        tloge("nvx_memcpy_s erro!\n");
        free(revBuffer);
        return -1;
    }
    revBuffer->cmd = cmd;
    if (cmd == SET_SYS_XML && halXmlPtr != NULL) {
        ...
        temp = nvx_memcpy_s(revBuffer->xmlBuffer, HASH_FILE_MAX_SIZE, halXmlPtr->fileBuf, halXmlPtr->fileSize);
        if (temp != EOK) {
            tloge("nvx_memcpy_s erro!\n");
            free(revBuffer);
            return -1;
...
int CaDaemonConnectWithCaInfo(const CaAuthInfo *caInfo, int cmd, const TEEC_XmlParameter *halXmlPtr)
{
    ...
    if (nvx_memset_s(&message, sizeof(message), 0, sizeof(message)) != EOK) {
