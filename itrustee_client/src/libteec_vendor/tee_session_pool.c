    ...
    const TEEC_UUID *destination, uint32_t poolSize)
{
    ...
    (void)nvx_memset_s(sp, sizeof(*sp), 0, sizeof(*sp));
     ...
     */
    sp->context = context;
    if (nvx_memcpy_s(&sp->uuid, sizeof(TEEC_UUID), destination, sizeof(*destination)) != EOK) {
        free(sp);
        return NULL;
    }
...
static void DumpSessionState(const struct SessionPool *sp)
{
    ...
    (void)nvx_memset_s(array, len, 0, len);
...
static void DumpSessionPool(const struct SessionPool *sp)
{
    ...
    (void)nvx_memset_s(bitmap, BITS_PER_UINT32 + 1, 0, BITS_PER_UINT32 + 1);
