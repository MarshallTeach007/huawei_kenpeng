...
static int SetPathToBuf(uint8_t *buffer, uint32_t *len, uint32_t *inputLen, const char *path)
{
    ...
    ret = nvx_memcpy_s(buffer, *inputLen, &pathLen, sizeof(pathLen));
    if (ret != EOK) {
    ...
    *inputLen -= sizeof(pathLen);
    ...
    ret = nvx_memcpy_s(buffer, *inputLen, path, num);
    if (ret != EOK) {
...
static int SetUserNameToBuf(uint8_t *buffer, uint32_t *len, uint32_t *inputLen, const char *userName)
{
    ...
    ret = nvx_memcpy_s(buffer, *inputLen, &nameLen, sizeof(nameLen));
    if (ret != EOK) {
    ...
    *inputLen -= sizeof(nameLen);
    ...
    ret = nvx_memcpy_s(buffer, *inputLen, userName, num);
    if (ret != EOK) {
