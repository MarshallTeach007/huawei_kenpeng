...
int32_t LoadSecFile(int tzFd, FILE *fp, enum SecFileType fileType, const TEEC_UUID *uuid)
{
    ...
    do {
        ...
        if (uuid != NULL && nvx_memcpy_s((void *)(&ioctlArg.uuid), sizeof(ioctlArg.uuid), uuid, sizeof(*uuid)) != EOK) {
...
static void LoadLib(struct SecAgentControlType *secAgentControl)
{
    ...
    if (IsTaLib(&(secAgentControl->LibSec.uuid))) {
        ret =
            nvx_snprintf_s(fname, sizeof(fname), MAX_PATH_LEN - 1,
        ...
        ret = nvx_snprintf_s(fname, sizeof(fname), MAX_PATH_LEN - 1,
...
static void LoadTa(struct SecAgentControlType *secAgentControl)
{
    ...
    ret = nvx_snprintf_s(fname, sizeof(fname), MAX_PATH_LEN - 1, "%s/%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x.sec",
