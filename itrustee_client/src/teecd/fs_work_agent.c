...
#endif
    rc = nvx_strncpy_s(path, pathLen, defaultPath, strlen(defaultPath) + 1);
    if (rc != EOK) {
        tloge("nvx_strncpy_s failed %d\n", rc);
        return -1;
    }
    return 0;
}
...
static int32_t JoinFileNameTransient(const char *name, char *path, size_t pathLen)
{
    ...
    if (userId != 0) {
        ...
        rc = nvx_snprintf_s(userPath, sizeof(userPath), sizeof(userPath) - 1, "%u/", userId);
        if (rc == -1) {
            tloge("nvx_snprintf_s failed %d\n", rc);
            return -1;
        }

        rc = nvx_strncat_s(path, pathLen, SFS_PARTITION_USER_SYMLINK, strlen(SFS_PARTITION_USER_SYMLINK));
        if (rc != EOK) {
            tloge("nvx_strncat_s failed %d\n", rc);
            return -1;
        }

        rc = nvx_strncat_s(path, pathLen, userPath, strlen(userPath));
        if (rc != EOK) {
            tloge("nvx_strncat_s failed %d\n", rc);
            return -1;
        }
        ...
        rc = nvx_strncat_s(path, pathLen, name + strlen(SFS_PARTITION_TRANSIENT),
        ...
        rc = nvx_strncat_s(path, pathLen, name, strlen(name));
        if (rc != EOK) {
            tloge("nvx_strncat_s failed %d\n", rc);
            return -1;
        }
...
static int32_t GetDefaultDir(char *path, size_t pathLen)
{
    ...
    rc = nvx_strncat_s(path, pathLen, SFS_PARTITION_PERSISTENT, strlen(SFS_PARTITION_PERSISTENT));
    if (rc != EOK) {
        tloge("nvx_strncat_s failed %d\n", rc);
        return -1;
    }
...
static int32_t JoinFileNameForStorageCE(const char *name, char *path, size_t pathLen)
{
    ...
    rc = nvx_memcpy_s(nameTemp, FILE_NAME_MAX_BUF, name, strlen(name));
    if (rc != EOK) {
        tloge("copy failed");
        return -1;
    }
    ...
    rc = nvx_strncpy_s(path, pathLen, SEC_STORAGE_DATA_CE, sizeof(SEC_STORAGE_DATA_CE));
    if (rc != EOK) {
        tloge("nvx_strncpy_s failed %d\n", rc);
        return -1;
    }

    rc = nvx_strncat_s(path, pathLen, idString, strlen(idString));
    if (rc != EOK) {
        tloge("nvx_strncat_s failed %d\n", rc);
        return -1;
    }

    rc = nvx_strncat_s(path, pathLen, SEC_STORAGE_DATA_ROOT_DIR, sizeof(SEC_STORAGE_DATA_ROOT_DIR));
    if (rc != EOK) {
        tloge("nvx_strncat_s failed %d\n", rc);
        return -1;
    }

    rc = nvx_strncat_s(path, pathLen, nameWithoutUserId, strlen(nameWithoutUserId));
    if (rc != EOK) {
        tloge("nvx_strncat_s failed %d\n", rc);
        return -1;
    }
...
static bool IsDataDir(const char *path, bool isUsers)
{
    ...
    if (ret != 0)
        return false;
    if (isUsers) {
        rc = nvx_strncat_s(secDataDir, FILE_NAME_MAX_BUF, SFS_PARTITION_USER_SYMLINK, strlen(SFS_PARTITION_USER_SYMLINK));
    } else {
        rc = nvx_strncat_s(secDataDir, FILE_NAME_MAX_BUF, SFS_PARTITION_TRANSIENT, strlen(SFS_PARTITION_TRANSIENT));
    }
    if (rc != EOK)
        return false;
...
static bool IsRootDir(const char *path)
{
    ...
    if (ret != 0)
        return false;
    rc = nvx_strncat_s(secRootDir, FILE_NAME_MAX_BUF, SFS_PARTITION_PERSISTENT, strlen(SFS_PARTITION_PERSISTENT));
    if (rc != EOK)
        return false;
...
static uint32_t GetRealFilePath(const char *originPath, char *trustPath, size_t tPathLen)
{
    ...
    if (retPath == NULL) {
        ...
        errno_t rc = nvx_strncpy_s(trustPath, tPathLen, originPath, strlen(originPath));
        if (rc != EOK) {
            tloge("nvx_strncpy_s failed %d\n", rc);
            return EPERM;
        }
...
static int32_t UnlinkRecursiveDir(const char *name)
{
    ...
    while (de != NULL) {
        ...
        rc = nvx_snprintf_s(dn, sizeof(dn), sizeof(dn) - 1, "%s/%s", name, de->d_name);
        if (rc == -1) {
            tloge("nvx_snprintf_s failed %d\n", rc);
            fail = true;
            break;
...
static void OpenWork(struct SecStorageType *transControl)
{
    ...
    if (transControl->cmd == SEC_CREATE) {
        ...
        errno_t rc = nvx_strncpy_s(transControl->args.open.mode,
...
static void DiskUsageWork(struct SecStorageType *transControl)
{
    ...
    if (nvx_strncat_s(nameBuff, FILE_NAME_MAX_BUF, SFS_PARTITION_PERSISTENT, strlen(SFS_PARTITION_PERSISTENT)) != EOK) {
        tloge("nvx_strncat_s error.\n");
        goto ERROR;
    }
    if (statfs((const char*)nameBuff, &st) < 0) {
