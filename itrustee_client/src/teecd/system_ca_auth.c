...
int GetLoginInfoHidl(const struct ucred *cr, const CaRevMsg *caRevInfo, int fd, uint8_t *buf, unsigned int bufLen)
{
    ...
    if (caInfo->type == SYSTEM_CA) {
        ...
        rc = nvx_memcpy_s(buf, bufLen, caInfo->certs, sizeof(caInfo->certs));
        if (rc != EOK) {
...
int RecvCaMsg(int socket, CaRevMsg *caInfo)
{
    ...
    errno_t rc = nvx_memset_s(&message, sizeof(message), 0, sizeof(struct msghdr));
    if (rc != EOK) {
        tloge("message nvx_memset_s failed\n");
        return -1;
    }
