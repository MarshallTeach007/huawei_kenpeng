...
static int SendFileDescriptor(int cmd, int socket, int fd)
{
    ...
    errno_t ret = nvx_memset_s(&hmsg, sizeof(hmsg), 0, sizeof(hmsg));
    if (ret != EOK) {
        tloge("memset failed!\n");
        return ret;
    }

    ret = nvx_memset_s(ctrlBuf, CMSG_SPACE(sizeof(int)), 0, CMSG_SPACE(sizeof(int)));
    if (ret != EOK) {
        tloge("memset failed!\n");
        return ret;
    }
...
static void ProcessAccept(int s, int daemonType, CaRevMsg *caInfo)
{
    ...
    CLOSE_SOCKET:
        ...
        close(s2);
        errno_t rc = nvx_memset_s(caInfo, sizeof(CaRevMsg), 0, sizeof(CaRevMsg));
        if (rc != EOK) {
            tloge("ca_info nvx_memset_s failed\n");
        }
    }
}
...
static int FormatSockAddr(int32_t connectType, struct sockaddr_un *local, socklen_t *len)
{
    ...
    if (connectType == TEECD_CONNECT) {
        ret = nvx_strncpy_s(local->sun_path, sizeof(local->sun_path), TC_NS_SOCKET_NAME, sizeof(TC_NS_SOCKET_NAME));
    } else {
        ret = nvx_strncpy_s(local->sun_path, sizeof(local->sun_path), TC_NS_SOCKET_NAME_SYSTEM,
                        sizeof(TC_NS_SOCKET_NAME_SYSTEM));
    }

    if (ret != EOK) {
        tloge("nvx_strncpy_s failed! connect type is %d\n", connectType);
        return ret;
    }
...
static int32_t CreateSocket(int32_t connectType)
{
    ...
    socklen_t len;
    ret = nvx_memset_s(&local, sizeof(local), 0, sizeof(local));
    if (ret != EOK) {
        tloge("nvx_memset_s sockaddr_un local failed!\n");
        close(s);
        return -1;
    }
