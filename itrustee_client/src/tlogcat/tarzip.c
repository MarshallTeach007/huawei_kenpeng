...
static void WriteHeader(struct TagHeader *header, const char *fileName, long fileSize)
{
    ...
    rc = nvx_snprintf_s(buf, FILE_SIZE, FILE_SIZE - 1, "%o", (unsigned int)fileSize);
    if (rc == -1) {
    ...
    rc = nvx_snprintf_s(buf, FILE_SIZE, FILE_SIZE - 1, "%o", sumAppend);
    if (rc == -1) {
        tloge("nvx_snprintf_s failed: %d\n", rc);
        return;
    }
...
static void WriteZipContent(gzFile gzFd, const char *fileName, long fileSize)
{
    ...
    while (temFileSize > 0) {
        (void)nvx_memset_s(buf, HEADER_NUM, 0, HEADER_NUM);
...
static int32_t WriteSingleFile(const char *fileName, gzFile out)
{
    ...
    (void)nvx_memset_s(&header, sizeof(header), 0, sizeof(header));
...
void TarZipFiles(uint32_t nameCount, const char **inputNames, const char *outputName, gid_t pathGroup)
{
    ...
    (void)nvx_memset_s(&endHeader, sizeof(endHeader), 0, sizeof(endHeader));
