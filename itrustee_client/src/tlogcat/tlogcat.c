...
{
    int32_t ret = nvx_snprintf_s(name, nameLen, nameLen - 1, "%08X%04X%04X%02X%02X%02X%02X%02X%02X%02X%02X",
    ...
    FILE *file, long fileLen, uint32_t index)
{
    ...
    for (i = 0; i < LOG_FILES_MAX; i++) {
        ...
        rc = nvx_memcpy_s(g_files[i].logName, sizeof(g_files[i].logName), logName, strlen(logName) + 1);
        if (rc != EOK) {
            tloge("Memcpy log name failed\n");
            goto CLOSE_F;
        }

        rc = nvx_memcpy_s(&g_files[i].uuid, sizeof(g_files[i].uuid), uuid, sizeof(struct TeeUuid));
        if (rc != EOK) {
...
{
    if (nameAttr->isTa) {
        return nvx_snprintf_s(logName, logNameLen, strlen(LOG_FILE_TA_DEMO) + strlen(logPath), "%s%s%s-%u",
            logPath, "LOG@", nameAttr->uuidAscii, nameAttr->index);
    } else {
        return nvx_snprintf_s(logName, logNameLen, strlen(LOG_FILE_TEE_DEMO) + strlen(logPath), "%s%s-%u",
...
{
    if (nameAttr->isTa) {
        return nvx_snprintf_s(logName, logNameLen, strlen(LOG_FILE_TA_COMPRESS_DEMO) + strlen(logPath),
            "%s%s%s-%u.tar.gz", logPath, "LOG@", nameAttr->uuidAscii, nameAttr->index);
    } else {
        return nvx_snprintf_s(logName, logNameLen, strlen(LOG_FILE_TEE_COMPRESS_DEMO) + strlen(logPath),
...
static void ArrangeCompressFile(const struct TeeUuid *uuid)
{
    ...
    for (i = 1; i < LOG_FILE_INDEX_MAX; i++) {
        ...
        ret = nvx_memcpy_s(g_logName, sizeof(g_logName), g_logNameCompress, sizeof(g_logNameCompress));
        if (ret != EOK) {
            tloge("nvx_memcpy_s error %d\n", ret);
            return;
        }
    }
}
...
static void LogTmpDirClear(const char *tmpPath)
{
    ...
    while (de != NULL) {
        ...
        ret = nvx_snprintf_s(filePathName, sizeof(filePathName), sizeof(filePathName) - 1,
...
static void LogFilesCompress(const struct TeeUuid *uuid)
{
    ...
    rc = nvx_memcpy_s(&g_compressUuid, sizeof(g_compressUuid), uuid, sizeof(struct TeeUuid));
    if (rc != EOK) {
        tloge("nvx_memcpy_s error %d\n", rc);
        return;
    }
...
static int32_t LogFilesChecklimit(uint32_t fileNum)
{
    ...
    if (g_files[fileNum].fileLen >= LOG_FILE_LIMIT) {
        ...
        rc = nvx_memset_s(&g_files[fileNum], sizeof(g_files[fileNum]), 0, sizeof(struct LogFile));
        if (rc != EOK) {
...
static struct LogFile *GetUsableFile(const struct TeeUuid *uuid)
{
    ...
    for (i = 0; i < LOG_FILES_MAX; i++) {
        ...
        if (g_files[i].file == NULL) {
            ...
            (void)nvx_memset_s(&g_files[i], sizeof(g_files[i]), 0, sizeof(g_files[i]));
            continue;
        }
...
static void WritePrivateLogFile(const struct LogItem *logItem, bool isTa)
{
    ...
    if (writeNum != (size_t)logItem->logRealLen) {
        ...
        (void)nvx_memset_s(logFile, sizeof(struct LogFile), 0, sizeof(struct LogFile));
    }
...
static int32_t GetTeeLogPath(void)
{
    ...
    ret = nvx_snprintf_s(g_teePath, sizeof(g_teePath), sizeof(g_teePath) - 1,
    ...
    ret = nvx_snprintf_s(g_teeTempPath, sizeof(g_teeTempPath), sizeof(g_teeTempPath) - 1,
...
static int32_t Prepare(void)
{
    ...
    (void)nvx_memset_s(g_files, (sizeof(struct LogFile) * LOG_FILES_MAX), 0, (sizeof(struct LogFile) * LOG_FILES_MAX));
